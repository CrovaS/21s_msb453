...
# Real-Time Table has added to Reservation

# [START listRT]
def listRT(limit=10, cursor=None):
    cursor = int(cursor) if cursor else 0

    results = mongo.db.realtime.find(skip=cursor, limit=10).sort('title')
    books = builtin_list(map(from_mongo, results))
...
# [START readRT]
def readRT(id):
    result = mongo.db.realtime.find_one({'_id': _id(id)})
    return from_mongo(result)
# [END readRT]
...
# [START createRT]
def createRT(data):
    result = mongo.db.realtime.insert_one(data)
    return read(result.inserted_id)
# [END createRT]


# [START updateRT]
def updateRT(data, id):
    mongo.db.realtime.replace_one({'_id': _id(id)}, data)
    return read(id)
# [END updateRT]


def deleteRT(id):
    mongo.db.realtime.delete_one({'_id': _id(id)})
